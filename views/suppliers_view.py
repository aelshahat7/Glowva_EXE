"""
Suppliers screen - mirrors Customers, but for the purchase side.
"""
...
class SuppliersView(ctk.CTkFrame):
    ...
    def _build_search(self):
        self.search_entry = ctk.CTkEntry(
            self,
            placeholder_text=rtl("دوّري باسم المورد..."),
    ...
    def _refresh_list(self):
        ...
        if not suppliers:
            ctk.CTkLabel(
                self.list_frame,
                text=rtl("مفيش موردين يطابقوا البحث"),
                text_color="gray50"
            ).pack(pady=30)
            return

        for supplier in suppliers:
            self._render_row(supplier)
    ...
    def _render_row(self, supplier):
        row = ctk.CTkButton(
            ...
            height=64,
            anchor="e",
            command=lambda: self._open_detail(supplier)
        )
        row.pack(fill="x", pady=4)

        name = rtl(supplier["name"]) if supplier["name"] else rtl("بدون اسم")
        text = name
    ...
    def _open_detail(self, supplier):
        ...
        history_rows = [
            {
                "date_key": h["purchase_date"],
                "total": h["total"],
                "sub_key": h["invoice_number"]
            }
            for h in history
        ]
        ...
        PartyDetailDialog(
            ...
            history_rows,
            {
                ...
                "history_label": rtl("فواتير التوريد"),
                "sub_label": rtl("رقم الفاتورة"),
            }
        )
