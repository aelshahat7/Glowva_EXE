...
# Product is on the right, total is on the left.
...
class PurchasesView(ctk.CTkFrame):
    ...
    def _build_header(self):
        ctk.CTkLabel(
            self, text=rtl("المشتريات"), font=ctk.CTkFont(size=24, weight="bold")
    ...
    def _build_form(self, parent):
        ...
        ctk.CTkLabel(
            form, text=rtl("فاتورة توريد جديدة"), font=ctk.CTkFont(size=16, weight="bold")
        ...
        r += 1
        ctk.CTkLabel(form, text=rtl("المورد")).grid(row=r, column=1, padx=10, pady=6, sticky="e")
        self.supplier_combo = ctk.CTkComboBox(
            form, values=self._supplier_names(), justify="right"
        )
        self.supplier_combo.set("")
        self.supplier_combo.grid(row=r, column=0, padx=10, pady=6, sticky="ew")

        r += 1
        ctk.CTkLabel(form, text=rtl("رقم الفاتورة")).grid(row=r, column=1, padx=10, pady=6, sticky="e")
        self.invoice_entry = ctk.CTkEntry(form, justify="right")
        self.invoice_entry.grid(row=r, column=0, padx=10, pady=6, sticky="ew")
        ...
        self.product_entry = ctk.CTkEntry(
            product_row,
            placeholder_text=rtl("اكتبي اسم صنف جديد أو دوّري بـ🔍"),
            justify="right",
        )
        ...
        r += 1
        ctk.CTkButton(
            form, text=rtl("أضف للفاتورة"), command=self._add_line
        ).grid(row=r, column=1, columnspan=2, padx=10, pady=(4, 10), sticky="ew")

        r += 1
        # Full-width invoice/cart inside the form.
        cart_container = ctk.CTkFrame(
            form, fg_color=("gray92", "gray17"), corner_radius=8
        )
    ...
    def _supplier_names(self):
        return [rtl(s["name"]) for s in db.list_suppliers()]

    def _supplier_name_from_display(self, value):
        return unrtl(value).strip()

    def _open_search(self):
        products = db.list_products()
        ProductSearchDialog(
            self.winfo_toplevel(), products, self._on_product_picked,
            subtitle_key="buy_price", subtitle_label=rtl("آخر سعر شراء")
        )
    ...
    def _add_line(self):
        ...
        if not product:
            self.line_error.configure(text=rtl("اكتبي اسم الصنف الأول"))
            return
    ...
    def _render_cart(self):
        ...
        self.total_label.configure(text=rtl(f"الإجمالي: {total:,.2f}"))
    ...
    def _save_purchase(self):
        ...
        supplier = self._supplier_name_from_display(self.supplier_combo.get())
        if not supplier:
            self.save_error.configure(text=rtl("لازم تكتبي اسم المورد"))
            return
    ...
    def _build_recent_panel(self, parent):
        ...
        ctk.CTkLabel(
            panel, text=rtl("آخر فواتير التوريد"), font=ctk.CTkFont(size=16, weight="bold")
    ...
    def _refresh_recent_purchases(self):
        ...
        if not purchases:
            ctk.CTkLabel(
                self.recent_frame,
                text=rtl("مفيش فواتير توريد لسه"),
                text_color="gray50",
            ).pack(pady=20)
            return
        ...
        for p in purchases:
            ...
            ctk.CTkLabel(
                top,
                text=rtl(p["supplier_name"]) if p["supplier_name"] else rtl("بدون اسم"),
