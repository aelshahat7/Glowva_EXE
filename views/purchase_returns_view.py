...
class PurchaseReturnsView(ctk.CTkToplevel):
    ...
    def _build_footer(self):
        ...
        self.save_button = ctk.CTkButton(
            frame, text=rtl("حفظ مرتجع المشتريات"), fg_color="#C0392B", hover_color="#922B21",
            command=self._save
        )
    ...
    def _load_invoices(self):
        ...
        values = []
        for inv in invoices:
            invoice_no = inv["invoice_number"] or "بدون رقم"
            label = rtl(f"فاتورة توريد {inv['id']} | {inv['supplier_name']} | {inv['purchase_date']} | {invoice_no} | {inv['total']:,.2f}")
    ...
    def _load_selected_invoice(self):
        ...
        if not purchase_id:
            ...
            purchase_id = self.invoice_map.get(label)
        if not purchase_id:
            self.invoice_info.configure(text=rtl("مفيش فواتير توريد متاحة للمرتجع"))
            return

        invoices = {x["id"]: x for x in rs.list_purchase_invoices()}
        inv = invoices.get(purchase_id)
        if inv:
            self.invoice_info.configure(
                text=rtl(f"المورد: {inv['supplier_name']} | التاريخ: {inv['purchase_date']} | إجمالي الفاتورة: {inv['total']:,.2f}")
            )
    ...
    def _save(self):
        ...
        try:
            rs.create_purchase_return(purchase_id, items, reason=self.reason_entry.get().strip(), return_date=date.today().isoformat())
