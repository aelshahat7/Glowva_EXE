...
class PartyDetailDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent, party, history_rows, config):
        ...
        self.title(f"{config['title_prefix']}: {party['name']}")
    ...
    def _build_header(self, party):
        ...
        ctk.CTkLabel(
            header, text=party["name"], font=ctk.CTkFont(size=20, weight="bold")
        ).pack(anchor="e")

        contact = party.get("phone") or party.get("contact_info")
        if contact:
            ctk.CTkLabel(header, text=contact, text_color="gray50").pack(anchor="e")
        if party.get("address"):
            ctk.CTkLabel(header, text=party["address"], text_color="gray50").pack(anchor="e")
    ...
    def _build_summary(self, party, history_rows):
        ...
        for i, (label, value) in enumerate([("الإجمالي", f"{total:,.2f}"), ("عدد العمليات", str(count))]):
            box = ctk.CTkFrame(summary, fg_color="transparent")
            box.grid(row=0, column=i, padx=15, pady=15, sticky="ew")
            ctk.CTkLabel(box, text=label, text_color="gray50", font=ctk.CTkFont(size=12)).pack(anchor="e")
            ctk.CTkLabel(box, text=value, font=ctk.CTkFont(size=18, weight="bold")).pack(anchor="e")
    ...
    def _build_history(self, history_rows, config):
        ...
        ctk.CTkLabel(
            panel, text=config["history_label"], font=ctk.CTkFont(size=15, weight="bold")
        ).grid(row=0, column=0, sticky="e", pady=(0, 8))

        if not history_rows:
            ctk.CTkLabel(panel, text="لسه مفيش عمليات", text_color="gray50").grid(
                row=2, column=0, pady=30)
            return

        sub_label = config.get("sub_label", "تفاصيل")
        ...
        for col, text in enumerate([sub_label, "الإجمالي", config["date_label"]]):
            ctk.CTkLabel(
        ...
        for row_index, h in enumerate(history_rows):
            ...
            values = [
                str(h.get("sub_key") or "—"),
                f"{h['total']:,.2f}",
                h["date_key"],
            ]
            for col, text in enumerate(values):
                ctk.CTkLabel(
                    scroll, text=text, anchor="e", fg_color=bg, font=ctk.CTkFont(size=12)
