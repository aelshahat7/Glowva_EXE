...
class InventoryView(ctk.CTkFrame):
    ...
    def __init__(self, parent):
        ...
        # الافتراضي: الأصناف التي رصيدها أكبر من صفر
    ...
    def _render_summary(self):
        ...
        low = len([
            item for item in self.all_items
            if 0 < item["current_stock"] <= item["low_stock_threshold"]
        ])
    ...
    def _update_pagination(self):
        ...
        self.page_label.configure(
            text=(
                f"صفحة {current_page} من {total_pages}"
                f"   |   {total} صنف"
            )
        )
    ...
    def _render_page(self):
        ...
        if not self.filtered_items:
            ...
            if self.current_filter == "available":
                msg = "مفيش أصناف عليها مخزون حاليًا"

            elif self.current_filter == "low":
                msg = "مفيش أصناف قرّبت تخلص 👍"
        ...
        page_items = self.filtered_items[
            start:end
        ]
