...
class ProfitPayoutView(ctk.CTkFrame):
    ...
    def _build_header(self):
        ctk.CTkLabel(
            self, text=rtl("صرف الأرباح"), font=ctk.CTkFont(size=24, weight="bold")
    ...
    def _render_summary(self):
        ...
        cards = [
            ...
            ("المتاح فعليًا", summary["available_balance"], "#C0392B" if summary["available_balance"] < 0 else "#27AE60"),
        ]

        for i, (label, value, color) in enumerate(cards):
            card = ctk.CTkFrame(self.summary_frame, corner_radius=10)
            card.grid(row=0, column=2 - i, padx=6, sticky="nsew")
            ctk.CTkLabel(card, text=rtl(label), font=ctk.CTkFont(size=13), text_color="gray50").pack(
                padx=15, pady=(15, 2), anchor="e")
            ctk.CTkLabel(card, text=f"{value:,.2f}", font=ctk.CTkFont(size=20, weight="bold"), text_color=color).pack(
                padx=15, pady=(0, 15), anchor="e")
    ...
    def _build_form(self, parent):
        ...
        ctk.CTkLabel(form, text=rtl("صرف جديد"), font=ctk.CTkFont(size=16, weight="bold")).grid(
            row=0, column=0, columnspan=2, padx=15, pady=(15, 15), sticky="e")

        ctk.CTkLabel(form, text=rtl("التاريخ")).grid(row=1, column=0, padx=15, pady=6, sticky="e")
        self.date_entry = ctk.CTkEntry(form, justify="right")
        self.date_entry.insert(0, date.today().isoformat())
        self.date_entry.grid(row=1, column=1, padx=15, pady=6, sticky="ew")

        ctk.CTkLabel(form, text=rtl("المبلغ")).grid(row=2, column=0, padx=15, pady=6, sticky="e")
        self.amount_entry = ctk.CTkEntry(form, justify="right")
        self.amount_entry.grid(row=2, column=1, padx=15, pady=6, sticky="ew")

        ctk.CTkLabel(form, text=rtl("السبب")).grid(row=3, column=0, padx=15, pady=6, sticky="e")
        self.reason_entry = ctk.CTkEntry(form, placeholder_text=rtl("اختياري"), justify="right")
        ...
        ctk.CTkButton(
            form, text=rtl("سجّل الصرف"), fg_color="#27AE60", hover_color="#1E8449",
            font=ctk.CTkFont(size=14, weight="bold"), height=38, command=self._save
    ...
    def _save(self):
        ...
        try:
            ...
            self.error_label.configure(text=rtl("المبلغ لازم يكون رقم"))
            return

        if amount <= 0:
            self.error_label.configure(text=rtl("المبلغ لازم يكون أكبر من صفر"))
            return
    ...
    def _build_history(self, parent):
        ...
        ctk.CTkLabel(panel, text=rtl("آخر عمليات الصرف"), font=ctk.CTkFont(size=16, weight="bold")).pack(
            padx=15, pady=(15, 10), anchor="e")
    ...
    def _refresh(self):
        ...
        if not payouts:
            ctk.CTkLabel(self.history_frame, text=rtl("لسه مفيش عمليات صرف"), text_color="gray50").pack(pady=20)
            return
        ...
        for p in payouts:
            ...
            ctk.CTkLabel(top, text=f"{p['amount']:,.2f}", font=ctk.CTkFont(weight="bold"), text_color="#E67E22").pack(side="left")
            ctk.CTkLabel(top, text=p["payout_date"], text_color="gray50", font=ctk.CTkFont(size=11)).pack(side="right")
            if p["reason"]:
                ctk.CTkLabel(row, text=rtl(p["reason"]), text_color="gray50", font=ctk.CTkFont(size=12), anchor="e").pack(fill="x", padx=10, pady=(0, 8))
            else:
